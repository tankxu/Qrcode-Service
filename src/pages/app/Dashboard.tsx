...
import { buildQrUrlDisplay } from "@/src/lib/qrUrl";
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <Link
          to="/new"
          className="inline-flex items-center justify-center h-10 px-4 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700"
        >
...
function QrCard({ qr }: { qr: Qr }) {
  ...
  return (
    <Link
      to={`/qr/${qr.id}`}
      className="group bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:border-blue-200 hover:shadow-md transition-all"
    >
      ...
      <p className="text-xs text-slate-500 mt-1 truncate">{buildQrUrlDisplay(qr.slug)}</p>
...
function EmptyState() {
  ...
  return (
    <div className="bg-white border border-dashed border-slate-300 rounded-2xl py-16 px-6 text-center">
      <div className="w-14 h-14 rounded-2xl bg-blue-50 text-blue-600 inline-flex items-center justify-center mb-4">
      ...
      <Link
        to="/new"
        className="inline-flex items-center justify-center h-10 px-5 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700"
      >
        <Plus className="w-4 h-4 mr-1.5" />
        {t("dashboard.newQr")}
      </Link>
